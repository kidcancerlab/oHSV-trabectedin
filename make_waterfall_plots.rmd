...
experiments <- list.files("Input_for_Waterfall_Plots")
#List to hold each experiment's combined dataframe
dfs_list <- list()
#Loop through experiments to download files
for(i in experiments) {
    #Var to hold sheets
    sheets <- paste("Input_for_Waterfall_Plots",
                    i,
                    list.files(paste0("Input_for_Waterfall_Plots/", i)),
                    sep = "/")
    sheet_df <- data.frame()
...
capture.output(resp_counts, file = "Data/Waterfall/resp_counts.txt")
```
...
write_xlsx(
    x = fisher_tests,
    path = "Data/Waterfall/fisher_test.xlsx"
)

...
tenx_load_qc <- function(path_10x, min_cells = 5, min_features = 800,
                         mt_pattern = "^mt-|^MT-|^hg19-MT-|^mm10-mt-",
                         species_pattern = "",
                         violin_plot = TRUE,
                         human_hsv = FALSE,
                         mouse_hsv = FALSE,
  ...
  if (human_hsv == TRUE) {
    raw_data <- raw_data[-grep(pattern = "^mm10", 
                               raw_data@Dimnames[[1]]), ]
    raw_data@Dimnames[[1]] <- sub("hg19_",
                                  "",
                                  x = raw_data@Dimnames[[1]])
  }
  if (mouse_hsv == TRUE) {
    raw_data <- raw_data[-grep(pattern = "^hg19", 
                               raw_data@Dimnames[[1]]), ]
    raw_data@Dimnames[[1]] <- sub("mm10_",
                                  "",
                                  x = raw_data@Dimnames[[1]])
  }
  if (mixed_hsv == TRUE &
      species_pattern == "") {
    print("Warning: Include species_pattern for mixed_hsv samples!")
  }
  if (mixed_hsv == TRUE &
      species_pattern != "") {
    raw_data@Dimnames[[1]] <- sub(species_pattern,
                                  "",
                                  x = raw_data@Dimnames[[1]])
  }
  if (human_hsv == FALSE &

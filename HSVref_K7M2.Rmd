...
sample_names <- c('S0190_HSV_exon_edits',
                  'S0191_HSV_exon_edits',
                  'S0192_HSV_exon_edits',
                  'S0193_HSV_exon_edits')
...
K7M2_hsv <- merge(Control,
                  ...
                  add.cell.ids = c("Control",
                                   ...
                                   "Trabectedin",
                                   "Combination"),
                  project = "Trabectedin plus HSV1716 in K7M2 - repeat")

#### Normalize, scale, process, cluster ####
K7M2_hsv <- NormalizeData(K7M2_hsv) %>%
  ScaleData() %>%
...
K7M2_hsv <- FindNeighbors(K7M2_hsv, dims = 1:15) %>%
...
K7M2_hsv <- FindClusters(K7M2_hsv,
                         resolution = 0.2) %>%
  RunUMAP(dims = 1:15)
...
library(data.table)
K7M2_hsv_even_d <- K7M2_hsv_even@meta.data %>%
  as.data.table

# the resulting md object has one "row" per cell
K7M2_hsv_even_d[, .N, by = "treatment"]

DimPlot(K7M2_hsv_even,
        split.by = "treatment") +
  coord_fixed()

save(K7M2_hsv_even, file = "Data/K7M2_hsv_even.RData")
```
...
Idents(K7M2_hsv_even) <- K7M2_hsv_even$RNA_snn_res.0.2
...
k7m2_ids <- DimPlot(K7M2_hsv_even,
  ...
  ggtitle("Mouse Reference")
k7m2_ids
pdf("k7m2_ids_dim.pdf", height = 5, width = 10)
k7m2_ids
dev.off()
```
...
pdf("k7m2_ab_vln.pdf", height = 5, width = 5)
k7m2_ifn_vln
dev.off()
...
pdf("k7m2_ifn_feat.pdf", height = 6, width = 12)
k7m2_ifn_feat
dev.off()
```
